...
public class AssetLineageOMRSTopicListener implements OMRSTopicListener {
    ...
    /**
     * Process new event for lineage entities.
     *
    ...
    private void processNewEntity(EntityDetail entityDetail) throws OCFCheckedExceptionBase, JsonProcessingException {
        if (!PROCESS.equalsIgnoreCase(entityDetail.getType().getTypeDefName())) {
            return;
        }
     ...
     * @throws JsonProcessingException exception parsing the event json
     */
    private void processUpdatedEntity(EntityDetail entityDetail) throws OCFCheckedExceptionBase,
